...
# Function responsible for initialing the syntax analyses 
    ...
    def boolfactor(self):
        if self._token._recognized_string == 'not':
            ...
            arg_2= self.expression()
            b_true=self.makeList(self.nextquad())
            self.genQuad(rel_op,arg_1, arg_2, '_')
...
class CCodeGenerator(Parser):
    ...
    def __init__(self, subprogram_flag = None,  c_code_file = None):
        super().__init__()
        self._subprogram_flag = subprogram_flag
    ...
    def generate_code(self):
        ...
        for quad in self._quads_list:
            if quad.op == 'begin_block':
                ...
                if self._ci_variables_list:
                    for ci_variable in self._ci_variables_list:
                        c_variable_line = c_variable_line + ci_variable + ',' + ' '
                if self._temp_variables_list:
                    for self._temp_variable in self._temp_variables_list:
...
#c code generator
    if parser._subprogram_flag == False:
        ...
        print('C-imple file has subprogram. C file generation aborted ')
