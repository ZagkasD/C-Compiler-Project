...
class Table(Lex):
    def __init__(self, scopes_list = list()):
    ...
    def add_variable(self,var_name):
        ...
        for entity in self._scopes_list[nesting_level]._entities_list:
            if entity._name == var_name:
                self.error('Variable << %s >> is already declared.' %var_name, self._line_number)
        #TODO Check if variable entity is a subprogram parameter
        var_offset = self._scopes_list[-1].get_next_offset()
        self._scopes_list[-1]._offset = var_offset
        self._scopes_list[-1]._entities_list.append(Variable(var_name,var_offset))
    ...
    def add_function(self,func_name):
        ...
        self._scopes_list[-1]._entities_list.append(Function(func_name))

    def add_parameter(self, par_name):
        par_offset = self._scopes_list[-1].get_next_offset()
        self._scopes_list[-1]._entities_list.append(Parameter(par_name,par_offset))
    ...
    # Update fields such as framelength or startingQuad which were not available at creation
    def update_field():
        pass
    ...
    def add_formal_param(self,par_name, mode):
        ...
        self._scopes_list[-1]._entities_list.append(FormalParameter(par_name, mode))
...
class Scope():
    def __init__(self, nesting_level = 0,):
        ...
        self._offset = 8                    # 12 bytes for return address, return value and access  
        # Here is 8 bytes because the add_variable method already calculates the next offset
        # Adding 4 more bytes. If we set the offset for scope at 12, then the add_variable method
        # Will set the offset for the first variable at 16, which should have been 12
...
class Constant(Entity):
    def __init__(self, name=None, datatype = int, value = None):
        super().__init__(name)
...
class Variable(Entity):
    def __init__(self, name=None,offset = 0):
        super().__init__(name)
        self._datatype = int # Always int for C-imple
        self._offset = offset     # Distace from start of activation record

class FormalParameter(Entity):
    def __init__(self, name=None, mode = None, datatype = int):
        super().__init__(name)
        self._mode = mode              # Parameter mode: cv(value), ref, ret
        self._datatype = datatype      # Always int for C-imple

class Parameter(Variable, FormalParameter):
    def __init__(self, name=None, offset=0, datatype=int):
        super().__init__(name, offset, datatype)

class TemporaryVariable(Variable):
    def __init__(self, name=None, datatype=int, offset=0):
...
class Function(Procedure):
    def __init__(self, name=None, datatype = int, startingQuad=None, framelength=0, formalParameters=list()):
        super().__init__(name, startingQuad, framelength, formalParameters)
        self._datatype = datatype   # Always int for C-imple

class SymbolicConstant():
    def __init__(self, name = None, datatype = int, value = 0):
        self._name = name
        self._datatype = datatype   # Always int for C-imple
        self._value = value
...
# Function responsible for initialing the syntax analyses 
    ...
    def subprogram(self): 
        if self._token._recognized_string == 'function':
            ...
            self.add_function(procedure_name)
            self.add_new_scope()
            if self._token._family == 'id':
                self.get_token()
                if self._token._recognized_string == '(':
    ...
    def formalparitem(self):
        if self._token._recognized_string == 'in':
            ...
            if self._token._family == 'id':
                ...
                self.add_parameter(self._token._recognized_string)
                self.get_token()
            else:
                self.error('Expected << id >>. Instead got << {0} >>'.format(self._token._recognized_string),self._line_number)
...
class CCodeGenerator(Parser):
    ...
    def __init__(self, subprogram_flag = None, quads_list = list()):
        super().__init__(quads_list)
        self._subprogram_flag = subprogram_flag
