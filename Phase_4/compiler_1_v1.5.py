...
class Parser(Lex):
    ...
    def gnlvcode(self,variable):
        ...
        foreign_entity= self.search_entity(variable) #TODO emfanish la8oys an den bre8ei to onoma ths metablhths
    ...
    def loadvr(self,variable, register_number):
        ...
        if isinstance(variable,int):
            ...
            if ((entity._datatype == 'Variable' and self.check_nesting_level(variable) == self._scopes_list[-1]._nesting_level)
                 or (entity._datatype == 'Parameter' and self._check_nesting_level(variable) == self._scopes_list[-1]._nesting_level and entity.mode == 'cv')
            ...
            elif(entity.datatype == 'Parameter' and self._check_nesting_level(variable) == self._scopes_list[-1]._nesting_level and entity.mode == 'ref'):
            ...
            elif((entity.datatype == 'Variable' and self.check_nesting_level(variable) < self._scopes_list[-1]._nesting_level)
                  or entity.datatype == 'Parameter' and self._check_nesting_level(variable) < self._scopes_list[-1]._nesting_level and entity.mode == 'cv'):
                self.gnlvcode(variable)
                self._assembly_file.write(' lw $t%s, ($t0)\n' % register_number)

            # Parameter with ref that belongs to an ancestor (smaller nesting level)
            elif(entity._datatype == 'Parameter' and self._check_nesting_level(variable) < self._scopes_list[-1]._nesting_level and entity._mode == 'ref'):
                self.gnlvcode(variable)
            ...
            # to the main program. 
            elif(entity._datatype == 'Variable' and self._check_nesting_level(variable) == 0): # nesting level = 0 because it's the main program
...
##@#1.2.3.3 local var, ancestor parameter with ref
    ...
    def final_code_generator(self,quad,block_name):
        ...
        #call
            ...
            if self.actual_pars:
                if self._actual_pars[-1].arg2 == 'RET':
                    self._actual_pars.pop()
                else:
                    self._assembly_file.write('addi $fp, $sp, %d\n' % framelength)
            #arguments check
            if len(to_call.arguments_list) != len(self._actual_pars):
                self.error('Subprogram \'%s\' arguments do not match definition' % to_call.name, -1, -1)
                #TODO check
...
def main():
    ...
    input_file_name = "_power.ci"
    input_file = open('_power.ci','r')
